...
def plot_costs(eval_costs, train_costs, num_epochs):
    ...
    graph.plot(t, eval_costs, c='g')
    graph.plot(t, train_costs, c='b')
...
def plot_accuracies(eval_accs, train_accs, num_epochs):
    ...
    fig = plt.figure()
    graph = fig.add_subplot(111)
    graph.plot(t, eval_accs, c='g')
    graph.plot(t, train_accs, c='b')
...
def collect_1_hidden_layer_net_stats(lower_num_hidden_nodes,
                                     ...
                                     eval_data):
    # your code here
    l = lower_num_hidden_nodes
    u = upper_num_hidden_nodes
    bestAcc = 0

    print("\n\n    COLLECTION 1\n\n\n")
    
    netResults = {}
    for n in range(l,u+1, 10):
        print(f"\nCurrent set is {n}\n\n")
        net = network2.Network([784, n, 10], cost=network2.CrossEntropyCost)
        net_stats = net.SGD(train_data, num_epochs, mbs, eta, lmbda=lmbda,
                                evaluation_data=eval_data,
                                monitor_evaluation_cost=True,
                                monitor_evaluation_accuracy=True,
                                monitor_training_cost=True,
                                monitor_training_accuracy=True)
        netResults[n] = net_stats
        if(net_stats[-3][-1] > bestAcc):
            cPickle.dump(net, open("net1.pck", "wb"))
            bestANN = n
            bestAcc = net_stats[-3][-1]

            print(f"\nThe current (1) best ANN is {bestANN}\n")
            print(netResults[bestANN][-3][-1])
            print("\n\n")
    print(f"\nThe best ANN was {bestANN}\n\n")
    print(netResults[bestANN][-3][-1])
    print("")
    return netResults
...
def collect_2_hidden_layer_net_stats(lower_num_hidden_nodes,
    ...
    netResults = {}
    for n2 in range(l, u+1, 10):
        for n1 in range(l,u+1, 10):
            print(f"\nCurrent set is {n1}_{n2}\n\n")

            net = network2.Network([784, n1, n2, 10], cost=network2.CrossEntropyCost)
            net_stats = net.SGD(train_data, num_epochs, mbs, eta, lmbda=lmbda,
                                    evaluation_data=eval_data,
                                    monitor_evaluation_cost=True,
                                    monitor_evaluation_accuracy=True,
                                    monitor_training_cost=True,
                                    monitor_training_accuracy=True)
            netResults[str(n1)+'_'+str(n2)] = net_stats
            if(net_stats[-3][-1] > bestAcc):
                cPickle.dump(net, open("net2.pck", "wb"))
                bestANN = str(n1)+'_'+str(n2)
                bestAcc = net_stats[-3][-1]

                print(f"\nThe current (2) best ANN is {bestANN}\n")
                print(netResults[bestANN][-3][-1])
                print("\n\n")
    print(f"\nThe best ANN was {bestANN}\n\n")
    print(netResults[bestANN][-3][-1])
...
def collect_3_hidden_layer_net_stats(lower_num_hidden_nodes,
    ...
    for n3 in range(l,u+1, 30):
        for n2 in range(l,u+1, 30):
            for n1 in range(l,u+1, 30):
                print(f"\nCurrent set is {n1}_{n2}_{n3}\n\n")

                net = network2.Network([784, n1, n2, n3, 10], cost=network2.CrossEntropyCost)
                net_stats = net.SGD(train_data, num_epochs, mbs, eta, lmbda=lmbda,
                                        evaluation_data=eval_data,
                                        monitor_evaluation_cost=True,
                                        monitor_evaluation_accuracy=True,
                                        monitor_training_cost=True,
                                        monitor_training_accuracy=True)
                netResults[str(n1)+'_'+str(n2)+'_'+str(n3)] = net_stats
                if(net_stats[-3][-1] > bestAcc):
                    cPickle.dump(net, open("net3.pck", "wb"))
                    bestANN = str(n1)+'_'+str(n2)+'_'+str(n3)
                    bestAcc = net_stats[-3][-1]
                    
                    print(f"\nThe current (3) best ANN is {bestANN}\n")
                    print(netResults[bestANN][-3][-1])
                    print("\n\n")
                # if(len(netResults) >=3):
...
print(net_stats)
print(net_stats[-1][-1])
plot_costs(net_stats[0], net_stats[2], len(net_stats[0]))
plot_accuracies(net_stats[1], net_stats[3], len(net_stats[0]))

## best 96.66% accurate 784_100_10
a = collect_1_hidden_layer_net_stats(30, 100, network2.CrossEntropyCost, 30, 10, 0.5, 1.0, train_d, test_d)

## best 97.19% accurate 784_90_70_10
b = collect_2_hidden_layer_net_stats(30, 100, network2.CrossEntropyCost, 30, 10, 0.5, 1.0, train_d, test_d)

## best 97.2% accurate 784_100_30_30_10
c = collect_3_hidden_layer_net_stats(30, 100, network2.CrossEntropyCost, 30, 10, 0.5, 1.0, train_d, test_d)
