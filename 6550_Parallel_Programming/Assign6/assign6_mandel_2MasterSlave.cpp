...
int main(int argc, char **argv)
{
  ...
  if (rank == 0)
  {
    ...
    while (!done)
    {
      if (activeTasks < size - 2 && currentRow < rows)
      {
        // Send a task
      ...
      else if (activeTasks > 0)
      {
        ...
        MPI_Recv(&finishedProcess, 1, MPI_INT, MPI_ANY_SOURCE, 0, MCW, MPI_STATUS_IGNORE);

        // queue waiting processor
        availableProcesses.push(finishedProcess);
        activeTasks--;
      }
      else
      {
  ...
  else
  {
    ...
    while (!done)
    {
      ...
      MPI_Send(&*mandWork, cols, MPI_INT, 1, row, MCW);
      //Tell master you need work
